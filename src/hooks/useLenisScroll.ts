...
export function useLenisScroll() {
  ...
  useEffect(() => {
    ...
    // Renders at full rate regardless of reduced motion — deliberately not
    // capped (a prior version throttled this to 30fps there); the scene
    // itself is already what's toned down for reduced motion (no tech
    // sphere/gallery, M camera-locked), so a lower render rate on top of
    // that would only cost smoothness for no perceptible motion reduction.
